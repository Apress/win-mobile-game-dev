...
namespace GameEngineCh4
{
    public abstract class CGameEngineGDIBase : CGameEngineBase
    {
        ...
        // A dictionary of graphics for display within the game
            ...
            : base(gameForm)   
        {
            ...
            if (!CGameFunctions.IsSmartphone())
            {
                ...
                // Add event handler (if required)
                _inputPanel.EnabledChanged += new System.EventHandler(SIPEnabledChanged);
            }

            // Ensure that the form is initally painted in full.
            // Without this we end up with the previous screen content remaining in place.
            ForceRepaint();
        }
        ...
        public override void Render()
        {
            ...
            try
            {
                ...
                if (!renderRect.IsEmpty)
                {
                    ...
                    clipRegion = new Region(renderRect);
                    ...
                    if (_backgroundImage != null)
                    {
                        ...
                        backGfx.DrawImage(_backgroundImage, renderRect, renderRect, GraphicsUnit.Pixel);
                    }
                    else
                    {
                    ...
                    foreach (CGameObjectGDIBase gameObject in GameObjects)
                    {
                        ...
                        if (!gameObject.Terminate)
                        {
                            ...
                            if (gameObject.GetRenderRectangle().IntersectsWith(renderRect))
                            {
                                gameObject.Render(backGfx);
                            }
                        }
                    }
                    ...
                    //{
                    //    backGfx.DrawRectangle(p, renderRect.X, renderRect.Y, renderRect.Width - 1, renderRect.Height - 1);
                    //}

                    // Invalidate the render rectangle on the form so that it repaints
                    GameForm.Invalidate(renderRect);

                    // Add the render rect to the rectangle to be presented
                    _presentRect = CGameFunctions.CombineRectangles(_presentRect, renderRect);
                }
            }
            finally
            {
    ...
    private Rectangle FindCurrentRenderRectangle()
    {
        ...
        private void GameFormResize(object sender, EventArgs e)
        {
            ...
            if (_backBuffer == null || GameForm.ClientSize.Width != _backBuffer.Width || GameForm.ClientSize.Height != _backBuffer.Height)
            {
                // Re-prepare the game.
                Prepare();
                // Force the whole form to repaint
