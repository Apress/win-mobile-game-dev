...
namespace GameEngine
{
    public abstract class CGameEngineBase
    {
        ...
        /// <summary>
        /// A CHighScores instance that can be used for storing high score records
        /// </summary>
        private CHighScores _highScores;
        /// <summary>
        /// A CMessageBox instance that can be used for displaying messages to the user
        /// </summary>
        private CMessageBox _messageBox;
        /// <summary>
        /// A CAboutBox instance that can be used for displaying information to the user
        /// </summary>
        private CAboutBox _aboutBox;
        /// <summary>
        /// A CSettings instance that can be used for setting and retrieving game settings
        /// </summary>
        ...
        public CGameEngineBase(Form gameForm)
        {
            ...
            _highScores = new CHighScores();
            // Create the MessageBox object
            _messageBox = new CMessageBox();
            // Create the AboutBox object
            _aboutBox = new CAboutBox();
        ...
        /// <summary>
        /// Return our HighScores object
        /// </summary>
        public CHighScores HighScores
        {
            get { return _highScores; }
        }

        /// <summary>
        /// Return our MessageBox object
        /// </summary>
        ...
        /// <summary>
        /// Return our AboutBox object
        /// </summary>
        public CAboutBox AboutBox
        {
        ...
        public Random Random
        {
            get
            {
                // If the generator has not yet been created, create it now
        ...
        {
            get { return CGameFunctions.IsSmartphone(); }
        }
        ...
        /// <summary>
        /// Initialize the system timer that we will use for our game
        /// </summary>
        ...
        public virtual void Advance()
        {
            ...
            while (_updateElapsedTime >= _updateTime)
            {
                ...
                // Copy the objects to an array before looping so that we can modify the collection
                // during the loop.
                foreach (CGameObjectBase gameObj in _gameObjects.ToArray())
                {
        ...
        /// <summary>
        /// Calculate the interpolation factor for the current elapsed time
        /// </summary>
        /// <returns></returns>
        protected float GetInterpFactor()
        {
            return _updateElapsedTime / _updateTime;
        }
        ...
        public Capabilities CheckCapabilities(Capabilities requiredCaps)
        {
            ...
            if ((requiredCaps & Capabilities.Accelerometer) > 0 && !CheckCapabilities_CheckForAccelerometer()) missingCaps |= Capabilities.Accelerometer;

            // Check hardware capabilities
            if ((requiredCaps & Capabilities.OpenGL) > 0 && !CheckCapabilities_CheckForOpenGL()) missingCaps |= Capabilities.OpenGL;
        ...
        /// <summary>
        /// Detect whether a supported accelerometer is available in this device
        /// </summary>
        /// <returns>Returns true if a supported accelerometer is found, false if not.</returns>
        /// <remarks>This function uses reflection to check the accelerometer so that no
        /// reference to Sensors.dll needs to be added to the Game Engine. This saves
        /// us from having to distribute Sensors.dll with projects that don't use it.</remarks>
        private bool CheckCapabilities_CheckForAccelerometer()
        {
            Assembly sensorAsm;
            Type sensorType;
            MethodInfo sensorMethod;
            object accelSensor;

            try
            {
                // Attempt to load the Sensors dll
                sensorAsm = Assembly.LoadFrom("Sensors.dll");

                // Find the GSensorFactory type
                sensorType = sensorAsm.GetType("Sensors.GSensorFactory");
                // If the type was not found, throw an exception.
                // This will be picked up by our catch block and reported back to the user.
                if (sensorType == null) throw new Exception();

                // Find the CreateGSensor method within the Sensors.GSensorFactory object
                sensorMethod = sensorType.GetMethod("CreateGSensor");

                // Invoke the method so that it attempts to create out sensor object.
                // If no supported accelerometer is available, this will return null,
                // otherwise we will receive a sensor object.
                accelSensor = sensorMethod.Invoke(null, null);

                // Did we get an object?
                if (accelSensor == null)
                {
                    // No, so no supported accelerometer is available
                    return false;
                }

                // A supported accelerometer is available
                return true;

            }
            catch
            {
                // The Sensors.dll failed to load or did not contain the expected classes/methods.
                // Throw an exception as we have an application configuration problem rather than
                // just a missing capability.
                throw new Exception("Unable to load Sensors.dll, which is required by this application. Please ensure the application is installed correctly.");
            }
        }

        /// <summary>
        /// Detect whether OpenGL support is available in this device
        /// </summary>
        /// <returns>Returns true if OpenGL support is found, false if not.</returns>
        /// <remarks>This function uses the OpenGLES DLL to perform its check.
        /// Note however that unless this function is called, there is no dependency
        /// on the OpenGLES DLL being deployed with the application.</remarks>
        private bool CheckCapabilities_CheckForOpenGL()
        {
            OpenGLES.EGLDisplay eglDisplay;
            // Try to create an EGL display for our game form
            try
            {
                // Attempt to create an EGL display object
                eglDisplay = OpenGLES.egl.GetDisplay(new OpenGLES.EGLNativeDisplayType(GameForm));
                // Destroy the object that we created -- we were just testing
                // whether the call succeed and don't actually need the object.
                OpenGLES.egl.Terminate(eglDisplay);
        ...
        public String ReportMissingCapabilities(Capabilities missingCaps)
        {
            ...
            if ((missingCaps & Capabilities.Accelerometer) > 0)
                ret.Append("- requires a supported accelerometer, which was not found.\n");

            // Check hardware capabilities
            if ((missingCaps & Capabilities.OpenGL) > 0)
                ret.Append("- requires accelerated OpenGL support, which was not found.\n");
        ...
        /// <summary>
        /// Determine whether any GameEngine-provided modal panels are displayed.
        /// </summary>
        /// <returns></returns>
        public bool IsModalPanelDisplayed()
        {
            // Is the Highscore panel displayed?
            if (_highScores.IsDisplayed) return true;
            if (_messageBox.IsDisplayed) return true;
            if (_aboutBox.IsDisplayed) return true;

            // No panels are displayed
            return false;
        }
