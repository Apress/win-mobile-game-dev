...
namespace OpenGLES
{
    public class glu
    {
        ...
        unsafe public static void LookAt(float eyex, float eyey, float eyez,
                        float centerx, float centery, float centerz,
                        float upx, float upy, float upz)
        {
            float[] forward = new float[3], side = new float[3], up = new float[3];
            ...
            fixed (float* matrixPointer = &m[0, 0])
            {
                gl.MultMatrixf(matrixPointer);
            }
