...
namespace OpenGLES
{
    ...
    public class GlyphRun
    {
        ...
        void BuildLine(OpenGLFont font, string text, int startIndex, int x, int y)
        {
        ...
        public GlyphRun(OpenGLFont font, string text)
            : this(font, text, new Size(int.MaxValue, int.MaxValue), OpenGLTextAlignment.Left, true)
        {
        }

        public GlyphRun(OpenGLFont font, string text, Size size, OpenGLTextAlignment alignment, bool autoEllipsis)
        {
            mySize = size;
            Font = font;
            ...
            int totalWidth = 0;
            int totalChars = 0;
            LineBreak lineBreak = FitString(processingText, 0, size.Width);

            while (lineBreak.Index != processingText.Length)
            {
                LineBreak nextBreak = FitString(processingText, lineBreak.Index, size.Width);
                // see if this line needs ellipsis
                if (Font.myHeight + Font.myHeight + totalHeight > size.Width && autoEllipsis)
                {
            ...
            int y = 0;
            int curChars = 0;
            foreach (LineBreak lbreak in linebreaks)
            {
                int x;
                ...
                else
                {
                    switch (alignment)
                    {
                        ...
                        case OpenGLTextAlignment.Right:
                            x = size.Width - lbreak.Width;
                            break;
                        case OpenGLTextAlignment.Center:
                            x = (size.Width - lbreak.Width) / 2;
                            break;
                        default:
                ...
                BuildLine(font, lbreak.Text, curChars, x, y);
                y += Font.myHeight;
                curChars += lbreak.Text.Length;
            }

            if (mySize.Width == int.MaxValue)
                mySize.Width = totalWidth;
            if (mySize.Height == int.MaxValue)
