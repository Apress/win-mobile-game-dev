...
namespace Orbits
{
    public partial class MainForm : Form
    {
        ...
        void Render()
        {
            ...
            gl.LoadIdentity();
            // Scale down so that the shapes are not too large
            gl.Scalef(0.1f, 0.1f, 0.1f);

            // Draw the sun
            RenderColorQuad(Color.Yellow);

            // Rotate for the the orbiting planet
            gl.Rotatef(_rotation, 0, 0, 1);
            // Translate into position for the planet
            gl.Translatef(0, 6, 0);
            // Draw the planet
            RenderColorQuad(Color.MediumBlue);

            // Rotate for the moon
            gl.Rotatef(_rotation * 4, 0, 0, 1);
            // Translate into position for the moon
            gl.Translatef(0, 3, 0);
            // Draw the moon
            RenderColorQuad(Color.Gray);
        ...
        /// <summary>
        /// Render a quad from a specified system color
        /// </summary>
        /// <param name="quadColor"></param>
        private void RenderColorQuad(Color quadColor)
        {
            float r, g, b;

            // Convert the Color's red, green and blue levels into values
            // compatible with OpenGL
            r = (float)quadColor.R / 255;
            g = (float)quadColor.G / 255;
            b = (float)quadColor.B / 255;

            // Create a color array using the specified color for
            // all of the vertices
            float[] quadColors = new float[] { r, g, b,
                                               r, g, b,
                                               r, g, b,
                                               r, g, b };
            // Render the quad
